window.Graph or= () ->
  background_color = '#B3767E'
  backward_select_color = '#00F'
  edge_line_color = '#F2B1BA'
  forward_select_color = '#000'
  mouseover_color = '#F00'
  ...
  init_main_graph = () ->
    cy = cytoscape {
      ...
      style: cytoscape.stylesheet().selector('node').css({
        ...
        'target-arrow-color': forward_select_color,
    ...
    cy.on 'select', (ev) ->
      ...
      iid = ev.cyTarget.id()
      cy.$("edge[source=\"#{ iid }\"]").css({'line-color': forward_select_color})
      cy.$("edge[target=\"#{ iid }\"]").css({'line-color': backward_select_color})
    ...
    cy.on 'mouseover', (ev) ->
      ...
      el.css({'line-color': mouseover_color})
